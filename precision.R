...
full_sync <- read.csv(paste0(fullSyncDir,"full_sync.csv"), header = TRUE, sep = ",")
dynamic <- read.csv(paste0(resultsDir,"distributedDynamic.csv"), header = TRUE, sep = ",")

full_sync<-subset(full_sync,full_sync$numberOfPredictors > 200)
dynamic <- subset(dynamic,dynamic$numberOfPredictors > 300)
static <- subset(static,static$numberOfPredictors > 300)
isolated <- subset(isolated,isolated$numberOfPredictors > 300)
