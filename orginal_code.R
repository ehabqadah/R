...
xrange<-range(c(0,30000))#max(isolated$predictionsNo)))
yrange<-range(c(0,max(isolated$predAc)))
...
for (i in 1:numberOfModels) {
  ...
  lines(models[[i]]$predictionsNo, models[[i]]$predAc , type="l", lwd=lineWidths[i],
